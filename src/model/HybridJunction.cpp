...
void HybridJunction::update_constant(SparseSystem& system,
...
void HybridJunction::update_solution(
  ...
  for (size_t i = 0; i < num_outlets; i++) {
    ...
    auto pressure_recovery_coeff = parameters[global_param_ids[3 * num_outlets + i]];
...
void HybridJunction::update_gradient(
  ...
  for (size_t i = 0; i < num_outlets; i++) {
    ...
    if (global_param_ids.size() / num_outlets > 3) {
      ...
      std::cout << "pressure_recovery_coeff: " << pressure_recovery_coeff << std::endl;
    }
    auto q_out = y[global_var_ids[3 + 2 * i]];
    ...
    if (global_param_ids.size() / num_outlets > 3) {
      jacobian.coeffRef(global_eqn_ids[i + 1],
                        global_param_ids[3 * num_outlets + i]) = 0.0;  // TODO: Set appropriate gradient
    }
