...
#include "DirIndepJunction.h"

void DirIndepJunction::setup_dofs(DOFHandler& dofhandler) {
...
void DirIndepJunction::update_constant(SparseSystem& system,
...
void DirIndepJunction::update_solution(
    SparseSystem& system, std::vector<double>& parameters,
    const Eigen::Matrix<double, Eigen::Dynamic, 1>& y,
    const Eigen::Matrix<double, Eigen::Dynamic, 1>& dy) {
  for (size_t i = 0; i < num_outlets; i++) {
    // Get parameters
    auto pressure_recovery_coeff = parameters[global_param_ids[2 * num_outlets + i]];
    auto q_out = y[global_var_ids[3 + 2 * i]];
    auto pressure_recovery_resistance = pressure_recovery_coeff * q_out;

    // Mass conservation
    system.C(global_eqn_ids[i + 1]) = -pressure_recovery_resistance * q_out;
    system.dC_dy.coeffRef(global_eqn_ids[i + 1], global_var_ids[3 + 2 * i]) =
        -2.0 * pressure_recovery_resistance;
  }
}

void DirIndepJunction::update_gradient(
  ...
  for (size_t i = 0; i < num_outlets; i++) {
    ...
    if (global_param_ids.size() / num_outlets > 2) {
      pressure_recovery_coeff = alpha[global_param_ids[2 * num_outlets + i]];
    }
    auto q_out = y[global_var_ids[3 + 2 * i]];
    ...
    residual(global_eqn_ids[i + 1]) =
        p_in - p_out - (resistance + pressure_recovery_resistance) * q_out -
        inductance * dq_out;
  }
}
